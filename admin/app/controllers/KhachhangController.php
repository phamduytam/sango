...
class KhachhangController extends Controller
{
	public function actionIndex()
	{
		...
		$khachhang = new KhachHangAR("searchList");
		if($word) $khachhang->word = $word;
		$content = $khachhang->searchList();
	...
	public function actionAdd()
	{
		$this->pageTitle = 'Khách hàng';
		$this->breadcrumb = array(
			'Khách hàng' => app()->baseUrl.'/khachhang',
			'Add'		=> ''
			);

		$model = new KhachHangAR();
		if (app()->request->getPost('KhachHangAR'))
		{
			// POSTデータの取得
			$data = request()->getPost('KhachHangAR');
	...
	public function actionEdit($id)
	{
		...
		$khachhang = new KhachHangAR();
		$model = $khachhang->findByPk($id);
		if(!$model)
			return ;
		if (app()->request->getPost('KhachHangAR'))
		{
	...
	{
		$model = KhachHangAR::model()->findByPk($id);
		if($model->delete())
